(function () {
  ...
  $(function () {
    ...
    $form.on('submit', async function (event) {
      ...
      const positionValue = fields.position.val() || null;
      const payload = {
        ...
        status: positionValue === 'MANAGER' ? 'PENDING_APPROVAL' : 'ACTIVE'
      };

      try {
        setLoading(true);
        console.log('[admin signup payload]', payload);
        const data = await window.apiService.post('/api/admin/signup', payload);
        const status = (data && data.status) || payload.status || 'ACTIVE';
        const isPending = status === 'PENDING_APPROVAL';
        const message = isPending
            ? '가입 요청이 접수되었습니다. 최고관리자 승인 후 로그인할 수 있습니다.'
            : '새 관리자 계정이 생성되었습니다.';
        const confirmText = isPending ? '확인' : '관리자 로그인으로 이동';
        showPopup({
          title: isPending ? '가입 요청 완료' : '등록 완료',
          message,
          confirmText,
          onConfirm: () => {
